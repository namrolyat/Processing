...
Deque<GPoint> dq = new ArrayDeque<GPoint>();
...
GPointsArray pArray = new GPointsArray(xAxes);
public GPlot plot;
...
void setup() {
  ...
  for (int i =0; i < xAxes; i++) {
    pArray.add(i,0);   // initialize the array
  }
...
void draw() {
  ...
  if (newData) {
    for (int i =0; i < xAxes; i++) {
      ...
      graphPoint.setXY(i,yAxes);
      plot.addPoint(graphPoint);
      //plot.addPoint(i, graphPoint); 
